use {
    crate::{
        ...
        performer::{
            endpoints::Endpoint, EndpointError, EndpointType, Performer,
        },
...
impl<T> Copy for InputValue<T> {}
impl<T> Clone for InputValue<T> {
    fn clone(&self) -> Self {
        Self {
            handle: self.handle,
            _marker: PhantomData,
...
impl<T> Copy for OutputValue<T> {}
impl<T> Clone for OutputValue<T> {
    fn clone(&self) -> Self {
        Self {
            handle: self.handle,
            _marker: PhantomData,
...
/*impl GetOutputValue for crate::value::types::Object {
    type Output<'a> = Result<&'a [u8], ()>;
