...
impl Saw {
	...
	fn compute(&mut self, count: i32, _inputs: &[&[Stereo<f32>]], outputs: &mut [&mut [Stereo<f32>]]) {
		...
		for output0 in zipped_iterators {
			...
			*output0 = ((Stereo{left: 2.0, right: 2.0} * Stereo{left: fRec1, right: fRec1} + Stereo{left: -1.0, right: -1.0}));
