...
class TransformData {
  TransformData({required this.width, required this.height, required this.left, required this.top, required this.alignment, required this.padding});
  double? width;
  double? height;
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (widget.data.width != null) {
          ...
          if (widget.data.alignment == Alignment.topLeft || widget.data.alignment == Alignment.centerLeft || widget.data.alignment == Alignment.bottomLeft) {
            left = widget.data.left;
          }

          // Center horizontal alignment
          if (widget.data.alignment == Alignment.topCenter || widget.data.alignment == Alignment.center || widget.data.alignment == Alignment.bottomCenter) {
            left = constraints.maxWidth / 2 - widget.data.width! / 2 + widget.data.left;
          }

          // Right horizontal alignment
          if (widget.data.alignment == Alignment.topRight || widget.data.alignment == Alignment.centerRight || widget.data.alignment == Alignment.bottomRight) {
        ...
        if (widget.data.height != null) {
          ...
          if (widget.data.alignment == Alignment.topLeft || widget.data.alignment == Alignment.topCenter || widget.data.alignment == Alignment.topRight) {
            top = widget.data.top;
          }

          // Center vertical alignment
          if (widget.data.alignment == Alignment.centerLeft || widget.data.alignment == Alignment.center || widget.data.alignment == Alignment.centerRight) {
            top = constraints.maxHeight / 2 - widget.data.height! / 2 + widget.data.top;
          }

          // Bottom vertical alignment
          if (widget.data.alignment == Alignment.bottomLeft || widget.data.alignment == Alignment.bottomCenter || widget.data.alignment == Alignment.bottomRight) {
...
class TransformWidgetEditing extends StatefulWidget {
  TransformWidgetEditing({required this.data, required this.child, required this.onUpdate, required this.onTap, required this.tree});
  ...
  void constrain(BoxConstraints constraints) {
    ...
    if (widget.data.width != null) {
      ...
      if (widget.data.alignment == Alignment.topLeft || widget.data.alignment == Alignment.centerLeft || widget.data.alignment == Alignment.bottomLeft) {
        ...
        if (widget.data.left > constraints.maxWidth - widget.data.width! - widget.data.padding.left - widget.data.padding.right) {
          widget.data.left = constraints.maxWidth - widget.data.width! - widget.data.padding.left - widget.data.padding.right;
        }
      }

      // Center alignment
      if (widget.data.alignment == Alignment.topCenter || widget.data.alignment == Alignment.center || widget.data.alignment == Alignment.bottomCenter) {
        if (widget.data.left < -constraints.maxWidth / 2 + widget.data.width! / 2) {
          widget.data.left = -constraints.maxWidth / 2 + widget.data.width! / 2;
        }

        if (widget.data.left > constraints.maxWidth / 2 - widget.data.width! / 2 - widget.data.padding.left - widget.data.padding.right) {
          widget.data.left = constraints.maxWidth / 2 - widget.data.width! / 2 - widget.data.padding.left - widget.data.padding.right;
        }
      }
    ...
    if (widget.data.height != null) {
      ...
      if (widget.data.alignment == Alignment.topLeft || widget.data.alignment == Alignment.topCenter || widget.data.alignment == Alignment.topRight) {
        ...
        if (widget.data.top > constraints.maxHeight - widget.data.height! - widget.data.padding.top - widget.data.padding.bottom) {
          widget.data.top = constraints.maxHeight - widget.data.height! - widget.data.padding.top - widget.data.padding.bottom;
        }
      }

      // Center alignment
      if (widget.data.alignment == Alignment.centerLeft || widget.data.alignment == Alignment.center || widget.data.alignment == Alignment.centerRight) {
        if (widget.data.top < -constraints.maxHeight / 2 + widget.data.height! / 2) {
          widget.data.top = -constraints.maxHeight / 2 + widget.data.height! / 2;
        }

        if (widget.data.top > constraints.maxHeight / 2 - widget.data.height! / 2 - widget.data.padding.top - widget.data.padding.bottom) {
          widget.data.top = constraints.maxHeight / 2 - widget.data.height! / 2 - widget.data.padding.top - widget.data.padding.bottom;
        }
      }
...
class Resizer extends StatefulWidget {
  Resizer({required this.data, required this.onUpdate, required this.child, required this.dragging});
  ...
  Widget build(BuildContext context) {
    return MouseRegion(
      ...
      child: Stack(
        children: [
          Padding(
            padding: (hovering || panning) ? const EdgeInsets.all(0) : const EdgeInsets.all(0),
            child: widget.child,
          ),
          Visibility(
            visible: hovering || panning,
            child: Padding(
              padding: (hovering || panning) && !widget.dragging ? const EdgeInsets.all(0) : const EdgeInsets.all(0),
              child: IgnorePointer(
                child: Container(
                  decoration: BoxDecoration(
                    border: Border.all(
          ...
          Visibility(
            maintainState: true,
            visible: (hovering || panning) && widget.data.width != null && !widget.dragging,
            child: Align(
              alignment: Alignment.centerRight,
              child: Padding(
          ...
          Visibility(
            maintainState: true,
            visible: (hovering || panning) && widget.data.height != null && !widget.dragging,
            child: Align(
              alignment: Alignment.topCenter,
              child: Padding(
          ...
          Visibility(
            maintainState: true,
            visible: (hovering || panning) && widget.data.width != null && widget.data.height != null && !widget.dragging,
            child: Align(
              alignment: Alignment.topLeft,
              child: GestureDetector(
                child: Container(
...
class TransformWidgetEditor extends StatefulWidget {
  TransformWidgetEditor({required this.data, required this.onUpdate, required this.tree});
            ...
            children: [
              Row(
                children: [
                  Field(
                    label: "WIDTH",
                    initialValue: widget.data.width == null ? "" : widget.data.width.toString(),
                    onChanged: (s) {
                  ...
                  Field(
                    label: "HEIGHT",
                    initialValue: widget.data.height == null ? "" : widget.data.height.toString(),
                    onChanged: (s) {
        ...
        Row(
          ...
          children: [
            AlignmentFieldButton(
              ...
              selected: alignment == Alignment.topLeft || alignment == Alignment.centerLeft || alignment == Alignment.bottomLeft,
              onTap: () {
                if (alignment == Alignment.topLeft || alignment == Alignment.topCenter || alignment == Alignment.topRight) {
                  onUpdate(Alignment.topLeft);
                } else if (alignment == Alignment.centerLeft || alignment == Alignment.center || alignment == Alignment.centerRight) {
            ...
            AlignmentFieldButton(
              ...
              selected: alignment == Alignment.topCenter || alignment == Alignment.center || alignment == Alignment.bottomCenter,
              onTap: () {
                if (alignment == Alignment.topLeft || alignment == Alignment.topCenter || alignment == Alignment.topRight) {
            ...
            AlignmentFieldButton(
              ...
              selected: alignment == Alignment.topRight || alignment == Alignment.centerRight || alignment == Alignment.bottomRight,
              onTap: () {
                if (alignment == Alignment.topLeft || alignment == Alignment.topCenter || alignment == Alignment.topRight) {
        ...
        Row(
          ...
          children: [
            AlignmentFieldButton(
              ...
              selected: alignment == Alignment.topLeft || alignment == Alignment.topCenter || alignment == Alignment.topRight,
              onTap: () {
                if (alignment == Alignment.topLeft || alignment == Alignment.centerLeft || alignment == Alignment.bottomLeft) {
                  onUpdate(Alignment.topLeft);
                } else if (alignment == Alignment.topCenter || alignment == Alignment.center || alignment == Alignment.bottomCenter) {
                  onUpdate(Alignment.topCenter);
            ...
            AlignmentFieldButton(
              ...
              selected: alignment == Alignment.centerLeft || alignment == Alignment.center || alignment == Alignment.centerRight,
              onTap: () {
                if (alignment == Alignment.topLeft || alignment == Alignment.centerLeft || alignment == Alignment.bottomLeft) {
                  onUpdate(Alignment.centerLeft);
                } else if (alignment == Alignment.topCenter || alignment == Alignment.center || alignment == Alignment.bottomCenter) {
            ...
            AlignmentFieldButton(
              ...
              selected: alignment == Alignment.bottomLeft || alignment == Alignment.bottomCenter || alignment == Alignment.bottomRight,
              onTap: () {
                if (alignment == Alignment.topLeft || alignment == Alignment.centerLeft || alignment == Alignment.bottomLeft) {
                  onUpdate(Alignment.bottomLeft);
                } else if (alignment == Alignment.topCenter || alignment == Alignment.center || alignment == Alignment.bottomCenter) {
...
class AlignmentFieldButton extends StatelessWidget {
  AlignmentFieldButton({required this.selected, required this.onTap, required this.iconData, this.borderRadius});
...
class _LabelDropdown extends State<LabelDropdown> with TickerProviderStateMixin {
...
class DualField extends StatefulWidget {
  DualField({required this.label, required this.field1, required this.field2, required this.fieldLabel1, required this.fieldLabel2, required this.onUpdate1, required this.onUpdate2});
            ...
            children: [
              ...
              if (!widget.hideIcon)
                RotationTransition(
                  ...
                  child: widget.icon ?? Icon(Icons.arrow_downward, color: Colors.white),
                ),
            ],
          ),
        ),
...
class Field extends StatefulWidget {
  Field({required this.label, required this.initialValue, required this.onChanged, this.width, this.height}) : super(key: UniqueKey());
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: const Color.fromRGBO(40, 40, 40, 1.0),
          border: Border.all(
            color: editing ? Colors.blue : const Color.fromRGBO(50, 50, 50, 1.0),
            width: 2.0
          ),
...
class FileField extends StatefulWidget {
  FileField({required this.path, required this.extensions, required this.onChanged, this.width, this.height}) : super(key: UniqueKey());
          ...
          children: [
            Expanded(
              child: Padding(
                ...
                child: TextField(
                  onSubmitted: widget.onChanged,
                  controller: TextEditingController(text: widget.path == null ? "" : widget.path!.split("/").last),
                  enabled: false,
                  style: const TextStyle(
                    color: Colors.grey,
                    fontSize: 14
...
class ColorField extends StatefulWidget {
  ColorField({required this.color, required this.onChanged, this.width, this.height}) : super(key: UniqueKey());
...
class _ColorField extends State<ColorField> {
  ...
  void initState() {
    super.initState();
    controller = TextEditingController(text: widget.color.toHex(leadingHashSign: false));
    focusNode.addListener(onFocusChange);
  }
...
class FieldLabel extends StatefulWidget {
  FieldLabel({required this.text, required this.child}) : super(key: UniqueKey());
...
class ChildDragTarget extends StatefulWidget {
  ChildDragTarget({this.width, this.height, this.child, required this.onAddChild, required this.tree, required this.host});
                            ...
                            child: ValueListenableBuilder<List<Var>>(
                              ...
                              builder: (context, vars, w) {
                                return Column(
                                  ...
                                  children: vars.map((e) => VarFieldElement(e, widget.varName)).toList()
                                );
                              },
                            )
                          )
