...
impl Voices {
    ...
    pub fn post<'a>(
        ...
        match self {
            Voices::Mono(performer) => performer.post(endpoint, event),
            Voices::Poly(performers) => {
                todo!()
            }
        }
    ...
    pub fn fetch(
        ...
        match self {
            Voices::Mono(performer) => performer.fetch(endpoint, callback),
            Voices::Poly(performers) => {
                todo!()
            }
        }
    ...
    pub fn copy_streams_to<T: StreamType>(
        &self,
        src_endpoint: Endpoint<OutputStream<T>>,
        dst_voices: &mut Self,
        dst_endpoint: Endpoint<InputStream<T>>,
        buffer: &mut [T],
    ) {
    ...
    pub fn copy_values_to<T>(
        &mut self,
        src_endpoint: Endpoint<OutputValue<T>>,
        dst_voices: &mut Self,
        dst_endpoint: Endpoint<InputValue<T>>,
    ) where
    ...
    pub fn copy_events_to(
        &mut self,
        src_endpoint: Endpoint<OutputEvent>,
        dst_voices: &mut Self,
        dst_endpoint: Endpoint<InputEvent>,
    ) {
        match (self, dst_voices) {
            (Voices::Mono(src_voice), Voices::Mono(dst_voice)) => {
                src_voice
