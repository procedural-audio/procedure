...
void Function(FFIWidgetPointer, double) ffiImageKnobSetValue = core
    .lookup<NativeFunction<Void Function(FFIWidgetPointer, Float)>>(
        "ffi_image_knob_set_value")
    .asFunction();
Pointer<Utf8> Function(FFIWidgetPointer) ffiImageKnobGetLabel = core
    .lookup<NativeFunction<Pointer<Utf8> Function(FFIWidgetPointer)>>(
        "ffi_image_knob_get_label")
    .asFunction();

class ImageKnobWidget extends ModuleWidget {
  ImageKnobWidget(Host h, FFINode m, FFIWidget w) : super(h, m, w) {
    _image = _loadImage(contentPath +
        "/assets/images/widgets/knobs/knob_small_scale_bipolar_128_frames.png");
  }
  ...
  Widget build(BuildContext context) {
    var labelRaw = ffiImageKnobGetLabel(widgetRaw.pointer);
    ...
    return Stack(children: [
      ...
      Container(
        ...
        child: FutureBuilder<ui.Image>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                return CustomPaint(
                  painter: KnobPainter(snapshot.data!, value),
                );
              } else {
      ...
      GestureDetector(
        onVerticalDragUpdate: (details) => setState(() {
          ...
          ffiImageKnobSetValue(widgetRaw.pointer, value);
...
class KnobPainter extends CustomPainter {
  KnobPainter(this.image, this.value);
